...
namespace dive {
	namespace weakform {
		...
		{
			const auto& elementMomentum = std::dynamic_pointer_cast<element::IElementMapped>(problemMomentum_->GetMesh()->GetElements()[element->GetElementIndex()]);

			auto numberNodes = elementMomentum->GetNumberNodes();
			auto numberDof = elementMomentum->GetNumberDof();
			auto dN = eilig::Inverse3x3(elementMomentum->J(local)) * elementMomentum->dN(local);
