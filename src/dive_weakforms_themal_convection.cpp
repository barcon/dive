...
namespace dive {
	namespace weakforms {
		...
		void ConvectionThermal::WeakFormulation(IElementPtr element, CacheIndex cacheIndex, const Vector& local, Matrix& output) const
		{
		...
		Scalar ConvectionThermal::FormDensity(IElementPtr element, const Vector& local) const
		{
		...
		Scalar ConvectionThermal::FormSpecificHeat(IElementPtr element, const Vector& local) const
		{
			auto temperature = values::GetValue(temperature_, local, element);
			auto pressure = values::GetValue(pressure_, local, element);
		...
		Matrix ConvectionThermal::FormVelocity(IElementPtr element, const Vector& local) const
		{
			return values::GetValueMatrix3D(velocity_, local, element);
		}
		Scalar ConvectionThermal::FormDivergence(IElementPtr element, const Vector& local) const
		{
		...
		Matrix ConvectionThermal::FormMatrix_N(IElementPtr element, const Vector& local, CacheIndex cacheIndex) const
		{	
			return element->N(local, cacheIndex);
		}
		Matrix ConvectionThermal::FormMatrix_dN(IElementPtr element, const Vector& local, CacheIndex cacheIndex) const
		{		
